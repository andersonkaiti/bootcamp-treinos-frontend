'use client'

import { cn } from '@lib/utils'
import { ArrowUp } from 'lucide-react'
import { RefObject } from 'react'

import { SUGGESTED_MESSAGES } from './use-chatbot'

interface ChatbotInputProps {
  input: string
  isLoading: boolean
  showSuggestions: boolean
  textareaRef: RefObject<HTMLTextAreaElement | null>
  onChange: (e: React.ChangeEvent<HTMLTextAreaElement>) => void
  onKeyDown: (e: React.KeyboardEvent<HTMLTextAreaElement>) => void
  onSend: () => void
  onSuggest: (text: string) => void
}

export function ChatbotInput({
  input,
  isLoading,
  showSuggestions,
  textareaRef,
  onChange,
  onKeyDown,
  onSend,
  onSuggest,
}: ChatbotInputProps) {
  const canSend = !!input.trim() && !isLoading

  return (
    <div className="flex shrink-0 flex-col gap-3">
      {showSuggestions && (
        <div className="flex gap-[10px] overflow-x-auto px-5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {SUGGESTED_MESSAGES.map((msg) => (
            <button
              key={msg}
              onClick={() => onSuggest(msg)}
              className="flex shrink-0 cursor-pointer items-center justify-center rounded-[100px] border-none bg-gray-200 px-4 py-2 font-[family-name:var(--font-inter-tight),sans-serif] text-sm leading-none whitespace-nowrap text-black"
            >
              {msg}
            </button>
          ))}
        </div>
      )}

      <div className="flex min-h-[82px] items-end gap-2 border-t border-gray-200 p-5">
        <textarea
          ref={textareaRef}
          value={input}
          onChange={onChange}
          onKeyDown={onKeyDown}
          placeholder="Digite sua mensagem"
          disabled={isLoading}
          rows={1}
          className="max-h-[120px] flex-1 resize-none overflow-y-auto rounded-2xl border border-gray-200 bg-gray-200 px-4 py-3 font-[family-name:var(--font-inter-tight),sans-serif] text-sm leading-[17px] text-black outline-none"
        />

        <button
          onClick={onSend}
          disabled={!canSend}
          className={cn(
            'mb-px flex h-[42px] w-[42px] shrink-0 items-center justify-center rounded-[99px] border-none transition-colors duration-150',
            canSend
              ? 'bg-primary cursor-pointer'
              : 'bg-primary/20 cursor-default'
          )}
          aria-label="Enviar mensagem"
        >
          <ArrowUp className="size-5 text-white" strokeWidth={1.6} />
        </button>
      </div>
    </div>
  )
}
